...
public class NetworkHelper {
    public static User createUser(String testName) {
        ...
        switch (testName) {
            ...
            case "approvedUserSuccessfulZCTransaction":
                request = BaseApi.buildGETRequest(Urls.APPROVEDUSER_WITHOTP
            ...
            case "approvedUserInsufficientCredit":
                request = BaseApi.buildGETRequest(Urls.APPROVEDUSER_INSUFFICIENTCREDIT
                        + "?phone_number=" + generatePhoneNo()
                        + "&integration_type=ZERO_CLICK"
                        + "&subscribe=true");
                break;
            case "approvedUserPendingBill":
                ...
                request = BaseApi.buildGETRequest(Urls.PENDINNG_DUES
                        + "?phone_number=" + generatePhoneNo()
                        + "&integration_type=ZERO_CLICK"
                        + "&subscribe=true"
                        + "&global_credit_limit=10000");
            ...
            case "nonExistingUserLienking":
                ...
                try {
                    ...
                    Response response = executeRequest(BaseApi.buildGETRequest(Urls.TEARDOWN_USER
    ...
    public static String getOTPFromVerificationId(String verification_id) throws IOException, JSONException {
        Request request = BaseApi.buildGETRequest(Urls.OTP_BY_VERIFICATION_ID + verification_id);
    ...
    public static void removeUser(String phoneNo) {
        try {
            executeRequest(BaseApi.buildGETRequest(Urls.TEARDOWN_USER
                    + "?phone_number=" + phoneNo));
            executeRequest(BaseApi.buildGETRequest(Urls.DELETE_USER_TOKEN + "/" + phoneNo));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
