...
import com.simpl.pay.sample.zc.objects.ItemObject;
...
public class CartActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void init() {
        content.add(new ItemObject("Awesome Product 1", 1, 10000));
        content.add(new ItemObject("Awesome Product 2", 2, 5000));
    ...
    public void onClickProceed() {
        if (getTotalAmount() == 0) {
            Toast.makeText(this, "Add atleast one item into the cart", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        payIntent.putExtra("has_token", getIntent().getBooleanExtra("has_token", false));
        payIntent.putExtra("merchant_id", getIntent().getStringExtra("merchant_id"));
